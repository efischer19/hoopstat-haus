...
def calculate_shooting_percentages(stats_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if (
        "field_goals_made" in stats_df.columns
        and "field_goals_attempted" in stats_df.columns
    ):
    ...
    if (
        "three_pointers_made" in stats_df.columns
        and "three_pointers_attempted" in stats_df.columns
    ):
    ...
    if (
        "free_throws_made" in stats_df.columns
        and "free_throws_attempted" in stats_df.columns
    ):
...
def calculate_season_to_date_stats(
    daily_stats_df: pd.DataFrame, existing_season_stats_df: pd.DataFrame = None
) -> pd.DataFrame:
    """
    ...
    if existing_season_stats_df is None or existing_season_stats_df.empty:
        ...
        season_stats = _accumulate_season_stats(
            daily_stats_df, existing_season_stats_df
        )
...
def _accumulate_season_stats(
    daily_stats_df: pd.DataFrame, existing_stats_df: pd.DataFrame
) -> pd.DataFrame:
    """
...
def validate_aggregated_data(stats_df: pd.DataFrame) -> dict[str, Any]:
    ...
    if null_player_ids > 0:
        validation_results["issues"].append(
            f"{null_player_ids} records with null player_id"
        )
        validation_results["validation_passed"] = False
    ...
    for col in numeric_columns:
        if col in stats_df.columns:
            ...
            if negative_count > 0:
                validation_results["issues"].append(
                    f"{negative_count} records with negative {col}"
                )
                validation_results["validation_passed"] = False
    ...
    for col in percentage_columns:
        if col in stats_df.columns:
            ...
            if invalid_pct > 0:
                validation_results["issues"].append(
                    f"{invalid_pct} records with invalid {col}"
                )
                validation_results["validation_passed"] = False
