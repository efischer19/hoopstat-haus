...
def test_mock_data_generation():
    ...
    dataset = generator.generate_complete_dataset(num_teams=3, players_per_team=4, num_games=6)
    
    # Validate counts
    assert len(dataset["teams"]) == 3, f"Expected 3 teams, got {len(dataset['teams'])}"
    assert len(dataset["players"]) == 12, f"Expected 12 players, got {len(dataset['players'])}"  # 3 * 4
    ...
    required_player_fields = ["id", "first_name", "last_name", "team_id", "height_inches", "weight_pounds"]
    for field in required_player_fields:
        assert field in sample_player, f"Missing player field: {field}"
    
    # Check game structure
    sample_game = games_data[0]
    required_game_fields = ["id", "home_team_id", "away_team_id", "home_score", "away_score", "game_date"]
...
def test_data_transformations():
    ...
    assert abs(height_meters - 1.8288) < 0.001, f"Height conversion incorrect: {height_meters}"
