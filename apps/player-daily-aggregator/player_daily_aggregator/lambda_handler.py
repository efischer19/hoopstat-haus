...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if player_games_df.empty:
            ...
            return create_response("success", "No data to process", correlation_id, 0, time.time() - start_time)
        ...
        if daily_stats_df.empty:
            ...
            return create_response("success", "No statistics generated", correlation_id, 0, time.time() - start_time)
        ...
        if not validation_results['validation_passed']:
            ...
            return create_response("error", "Data validation failed", correlation_id, records_processed, time.time() - start_time)
        
        # Write daily statistics to Gold layer
        gold_base_path = f"s3://hoopstat-haus-gold/player_daily_stats"
        ...
        for _, player_stats in daily_stats_df.iterrows():
            ...
            existing_season_df = s3_handler.read_existing_season_data(
                player_id, season, f"s3://hoopstat-haus-gold/player_season_stats"
            )
            ...
            season_base_path = f"s3://hoopstat-haus-gold/player_season_stats"
        ...
        log_error(
            error_msg,
            correlation_id,
            extra={'duration_in_seconds': duration, 'records_processed': records_processed}
        )
        
        return create_response("error", error_msg, correlation_id, records_processed, duration)
...
def log_warning(message: str, correlation_id: str, extra: Dict[str, Any] = None) -> None:
