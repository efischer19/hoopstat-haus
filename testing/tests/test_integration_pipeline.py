...
class TestFullPipelineIntegration:
    ...
    def test_medallion_architecture_setup(self, pipeline_runner):
        ...
        # Setup environment
        assert pipeline_runner.setup_environment(), "Failed to setup pipeline environment"
        ...
        for layer in ["bronze", "silver", "gold"]:
            ...
            assert pipeline_runner.s3_utils.bucket_exists(bucket_name), f"{layer} bucket does not exist"
    ...
    def test_bronze_layer_ingestion(self, pipeline_runner):
        ...
        assert pipeline_runner.ingest_bronze_data(num_teams, num_players_per_team), "Bronze ingestion failed"
        ...
        assert len(players_data["players"]) == num_teams * num_players_per_team, "Incorrect number of players"
        
        # Check games data
        games_data = pipeline_runner.s3_utils.get_object(bronze_bucket, "raw/games/games.json", "json")
        assert games_data is not None, "Games data not found"
        assert len(games_data["games"]) > 0, "No games data found"
        
        logger.info(f"✅ Bronze layer ingestion successful: {len(teams_data['teams'])} teams, "
    ...
    def test_silver_layer_transformation(self, pipeline_runner):
        ...
        assert len(players_df) == 12, "Incorrect number of players in silver layer"  # 3 * 4
        ...
        # Check games data
        games_df = pipeline_runner.s3_utils.get_object(silver_bucket, "cleaned/games/games.parquet", "dataframe")
        ...
        logger.info(f"✅ Silver layer transformation successful: {len(teams_df)} teams, "
    ...
    def test_gold_layer_aggregation(self, pipeline_runner):
        ...
        required_team_cols = ["team_name", "games_played", "wins", "losses", "win_percentage", "avg_points_per_game"]
        ...
        assert len(player_stats_df) == 6, "Incorrect number of players in gold layer"  # 2 * 3
        
        required_player_cols = ["player_name", "team_id", "position", "height_cm", "weight_kg", "bmi"]
        ...
        assert league_summary["total_players"] == 6, "Incorrect total players in summary"
        assert "avg_points_per_game" in league_summary, "Missing average points per game"
        
        logger.info(f"✅ Gold layer aggregation successful: {len(team_stats_df)} team stats, "
    ...
    def test_complete_pipeline_flow(self, pipeline_runner):
        ...
        assert pipeline_runner.run_full_pipeline(num_teams, num_players_per_team), "Full pipeline execution failed"
        ...
        for layer in ["bronze_layer", "silver_layer", "gold_layer"]:
            assert verification_results[layer]["status"] == "valid", f"{layer} validation failed"
    ...
    def test_pipeline_data_quality(self, pipeline_runner):
        ...
        # Bronze layer data
        bronze_teams = pipeline_runner.s3_utils.get_object(bronze_bucket, "raw/teams/teams.json", "json")
        bronze_players = pipeline_runner.s3_utils.get_object(bronze_bucket, "raw/players/players.json", "json")
        
        # Silver layer data
        silver_teams_df = pipeline_runner.s3_utils.get_object(silver_bucket, "cleaned/teams/teams.parquet", "dataframe")
        silver_players_df = pipeline_runner.s3_utils.get_object(silver_bucket, "cleaned/players/players.parquet", "dataframe")
        
        # Gold layer data
        gold_team_stats_df = pipeline_runner.s3_utils.get_object(gold_bucket, "metrics/team_stats.parquet", "dataframe")
        gold_player_stats_df = pipeline_runner.s3_utils.get_object(gold_bucket, "metrics/player_stats.parquet", "dataframe")
        
        # Data consistency checks
        assert len(bronze_teams["teams"]) == len(silver_teams_df), "Team count mismatch between bronze and silver"
        assert len(bronze_players["players"]) == len(silver_players_df), "Player count mismatch between bronze and silver"
        assert len(silver_teams_df) == len(gold_team_stats_df), "Team count mismatch between silver and gold"
        assert len(silver_players_df) == len(gold_player_stats_df), "Player count mismatch between silver and gold"
        
        # Data quality checks
        # Check that all teams have valid statistics
        assert gold_team_stats_df["games_played"].min() >= 0, "Invalid games played count"
    ...
    def test_pipeline_performance(self, pipeline_runner):
        ...
        logger.info(f"✅ Performance test passed with {num_teams} teams and {num_teams * num_players_per_team} players")
